...
public typealias JIntArray = JInt32Array

/// A Swift wrapper for a Java int array (`jintArray`).
///
/// Retains a global reference and provides access to individual `jint`s.
public final class JInt32Array: Java1DArrayable {
    ...
    public static func newArrayObject(_ env: JEnv, _ length: Int) -> jobject? {
        env.newIntArray(length: Int32(length))
    }
    #endif
    ...
    public static func setArrayRegion<C>(_ env: JEnv, _ object: JObject, _ collection: C) where C : ToJavaArrayIterable, ElementType == C.Element {
        ...
        env.setIntArrayRegion(object.ref.ref, start: 0, length: jsize(collection.count), buffer: &cArray)
        #endif
    }
    ...
    public static func getElementAtIndex(_ index: Int, _ object: JObject) -> ElementType? {
        ...
        env.getIntArrayRegion(object.ref.ref, start: jsize(index), length: 1, buffer: &result)
        return result
        #else
        return nil
        #endif
    ...
    public static func getAllElements(_ env: JEnv, _ object: JObject, _ length: Int) -> [ElementType] {
        ...
        env.getIntArrayRegion(object.ref.ref, start: 0, length: Int32(length), buffer: &result)
        return result
        #else
        return []
        #endif
...
public typealias JIntArray2D = JInt32Array2D

public final class JInt32Array2D: Java2DArrayable {
    public typealias ElementType = Int32

    public let object: JObject
    public let length: Int

    public static var elementClass: JClassName { "[I" }
